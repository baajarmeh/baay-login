...
module.exports = function(app) {
	...
	app.get('/', function(req, res){
		...
		if (req.cookies.user == undefined || req.cookies.pass == undefined) {
			...
			accounts.autoLogin(req.cookies.user, req.cookies.pass, function(o){
				if (o != null) {
				    req.session.user = o;
					res.redirect('/home');
				}
	...
	app.post('/lost-password', function(req, res){
		...
		accounts.getAccountByEmail(req.body['email'], function(o){
			if (o) {
	...
	app.get('/reset-password', function(req, res) {
		...
		accounts.validateResetLink(email, passH, function(e){
			if (e != 'ok') {
				res.redirect('/');
			}
			else {
	...
	app.post('/reset-password', function(req, res) {
		...
		req.session.destroy();
		accounts.updatePassword(email, nPass, function(e, o){
			if (o) {
				res.status(200).send('ok');
			}
			else {
	...
	app.post('/delete', function(req, res){
		accounts.deleteAccount(req.body.id, function(e, obj){
			if (!e) {
				res.clearCookie('user');
				res.clearCookie('pass');
				req.session.destroy(function(e){ res.status(200).send('ok'); });
	...
	app.get('/reset', function(req, res) {
		accounts.delAllRecords(function(){
			res.redirect('/print');
		});
	});
